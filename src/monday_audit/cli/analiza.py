"""Analiza konta jedną sesją: detektory → model → uwagi krytyczne (fazy 5b, 5c).
    ...
    uv run python -m monday_audit.cli.analiza --klient cxlabs --zakres workspace --id 7465500
    uv run python -m monday_audit.cli.analiza --zakres cale_konto --wejscie obraz.json
    uv run python -m monday_audit.cli.analiza --zakres cale_konto --tylko-szacunek
    uv run python -m monday_audit.cli.analiza --snapshot 1      # stary, zapisany snapshot
...
from monday_audit.agent.koszt import porownaj
...
from monday_audit.raport.uwagi import zapisz_html
...
from monday_audit.zbieranie.konto import zbuduj_zakres
