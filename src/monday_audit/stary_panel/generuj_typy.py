"""Generuje `front/src/api.ts` z kontraktu `pulpit.do_json()`.
    ...
    uv run python -m monday_audit.stary_panel.generuj_typy          # zapisz
    uv run python -m monday_audit.stary_panel.generuj_typy --sprawdz  # tylko sprawdź (CI)
...
from monday_audit.stary_panel.raport import Finding
...
from monday_audit.zbieranie.podglad_zakresu import PodgladKonta, TablicaDoWyboru, WorkspaceDoWyboru
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="monday_audit.stary_panel.generuj_typy")
    parser.add_argument(
        "--sprawdz",
        action="store_true",
    ...
    if argumenty.sprawdz:
        ...
        if argumenty.cel.read_text(encoding="utf-8") != tresc:
            print(
                ...
                f"uruchom `uv run python -m monday_audit.stary_panel.generuj_typy`",
                file=sys.stderr,
            )
            return 1
