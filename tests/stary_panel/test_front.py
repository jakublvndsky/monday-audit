...
from monday_audit.stary_panel.generuj_typy import zbuduj_tresc
...
MARKA_J2 = KORZEN / "src" / "monday_audit" / "raport" / "szablony" / "_marka.css.j2"
PULPIT_J2 = KORZEN / "src" / "monday_audit" / "raport" / "szablony" / "_pulpit.css.j2"
...
def test_typy_frontu_sa_aktualne() -> None:
    ...
    Gdy ten test padnie, uruchom:
        uv run python -m monday_audit.stary_panel.generuj_typy
    """
    ...
    assert API_TS.read_text(encoding="utf-8") == zbuduj_tresc(), (
        ...
        "uruchom `uv run python -m monday_audit.stary_panel.generuj_typy`"
    )
